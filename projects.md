...
I started by exploiting [Myspace](https://en.wikipedia.org/wiki/Myspace) by reverse engineering their api
to request a large number of local users listen to my music. They had basic captcha in place, so I
configured my script to download/display the image with a text prompt which became more and more frequent up
to the daily 500 user cap (or whatever it was - I seem to recall 500). They actually let you search by postal
code and age back then, so I first scraped a bunch of user ids based on my target demographics and created a
database to make requests. After using `Myspace` for a while, I came to the conclusion the experience was
broken due to a lack of consistency. They allowed users to customize their pages with CSS and HTML, which
presents all sorts of issues with compatibility and security.
...
When my ex-wife cheated on me and I was certain she did, I rooted her phone and installed sshd to gather
evidence. It was pretty simple, I think the SMS app was using SQLite.
...
Once I have my homelab set up (next project on this list), I want to build a distributed quantum computer simulator. Measurement may not be parallelizable (this isn't a word, is it?) but circuit construction certainly is.

## homelab

[This](https://github.com/jasoncolburne/homelab) is my most ambitious project to date. I built a secure
